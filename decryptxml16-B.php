...
/*
Step 1. Get the value from ?d= in browser($_GET['d']) -- This is the TMDB Original Title Name and Year
Step 2. Explode the qqqq to separate the 4 sections from the URL. The second section is teh TMDB and the third section is the year released.
Step 3. Manipulate the title to remove special characters including commas and apostrophe's.? Leave exclamation points and hyphans.
Step 4. XMLHTTPRequest the plex searchquery URL.
Step 5. Go through each response found. Only accept if it's a movie.
Step 6. If no results are found, scan by the first word in teh title. Go through all responses until one title is a match to the tmdb title. (Skip words like the, an, and, etc.
Step 7. use the urldecoded version of tmdb so that way spaces are shwon in title. then remove all special characters
...
*/
	...
	$link2 = explode ("qqqq", $link);  /*This is an array split into 4 sections from the url. part one is the encrypted base url, part two is the show title, part three is the tv show release year, part four is the seasonepsiode S03E05, and part five is the plex token. */
	...
	$link552 = str_replace("E","_E",$link552);

	$last_underscore = strrpos($link552, '_');
	$episode = substr($link552, $last_underscore);  // returns   _2
	$episode = substr($episode, 1);
	$season = substr($link552, 0, $last_underscore);  //  returns  3498_wizards_Wizards+of+Waverly+Place
	$season = substr($season, 1);	
	$episode = substr($episode, 1);

	/* print_r($season . $episode);  */
	...
	foreach($cont as $a=>$b){
        				/* Figure out how to make Flash or The Flash work. 2014?  possibly check if either titles start with 'the' then remove it from the title, then compare. */
        if($b->attributes()->type == 'show'){  /* Make sure that the item is a movie before scanning further. if its an episode it will be skipped.  */
            $d = $b->attributes()->title;   /*  $d is the Original Title the way that plex has it listed in their database.   */
            
            d = $b->attributes()->year;  /*  $dd is the year the movie was released according to Plex's database.   */
          ...
          /*  echo '<P> D= ' . preg_replace('/[^ \w]+/', '', $d) . ' link5=' . preg_replace('/[^ \w]+/', '', urldecode($link5)) . '<P>';   */
            if((strtolower(preg_replace('/[^ \w]+/', '', $d)) == strtolower(preg_replace('/[^ \w]+/', '', urldecode($link5)))) || (strtolower(preg_replace('/[^ \w]+/', '', $d)) == strtolower(preg_replace('/[^ \w]+/', '', (urldecode($link5)) . ' (' . $link55 . ')')))){  
          ...
          /*  if(($dd == $link55) or ($dd ==($link55+1)) or ($dd ==($link55-1))){    */      /* this line compares the years also plus or minus one year   -  remove this becase Movie House 2008 is showing American Pie Beta House 2007 movie - instead the line below just compares the plex release year to tmdb release year. No plus or minus one year. */   
            ...
            $coopforaye = substr($link7, 0, -13) . substr($e, 1)   . '?' . substr($link8, 1);     
           
           	$conttttt = @simplexml_load_file($coopforaye);  
               
            foreach($conttttt as $aaaaa=>$bbbbb){
                if($bbbbb->attributes()->index == $season){    
                	$eeeee = $bbbbb->attributes()->key;
                    $coopforaye2 = substr($link7, 0, -13) . substr($eeeee, 1)   . '?' . substr($link8, 1);  
                    
                    $conttttt2 = @simplexml_load_file($coopforaye2);  
                	foreach($conttttt2 as $aaaaa2=>$bbbbb2){
                        if($bbbbb2->attributes()->index == $episode){  
                        
                        
                
            
            foreach($bbbbb2 as $ween=>$peen){
                ...
                foreach($peen as $yeet=>$yeet2){    
                    ...
                    if($yeet2->attributes()->file != ''){
                        ...
                        if(strstr($fileName, '\\')){ $fileName = trim(substr($fileName, strrpos($fileName, '\\') + 1)); }   
                        if(strstr($fileName, '/')){ $fileName = trim(substr($fileName, strrpos($fileName, '/') + 1));   }
                        $newarr[0] = $fileName;
                    }
                    ...
                    if(($yeet2->attributes()->size < 22548578304)){
                        $final = ('<p> <a href="vlc://' . substr($link7, 0, -13) . substr($newarr[2], 1) . '?' . substr($link8, 1) . '" style="color:orange;">' . $newarr[0]  . '</a>   || <p  style="color:orange;">     FileSize: ' . round($newarr[1]/1024/1024/1024, 2) . 'GB  ||  Duration:  '. round($duration/60000) . ' mins   ||  BitRate: ' . round($bitrate/1000, 2)  . ' Mb/s || Video Codec: ' . $videoCodec . ' ||  Video Resolution: ' . $videoResolution . '</p>');
                        print_r($final);
                        $remember = $final;
                    }
                }
            ...
            foreach($b2 as $ween2=>$peen2){
                ...
                foreach($peen2 as $yeet2=>$yeet22){    
                    ...
                    if(($yeet22->attributes()->size < 22548578304)){
                        $final2 = ('<p> <a href="vlc://' . substr($link7, 0, -13) . substr($newarr[2], 1) . '?' . substr($link8, 1) . '" style="color:orange;">' . $newarr[0]  . '</a>   || <p style="color:orange;">     FileSize: ' . round($newarr[1]/1024/1024/1024, 2) . 'GB  ||  Duration:  '. round($duration2/60000) . ' mins   ||  BitRate: ' . round($bitrate2/1000, 2)  . ' Mb/s || Video Codec: ' . $videoCodec2 . ' ||  Video Resolution: ' . $videoResolution2 . '</p>');
                        ...
                        if($keepinmem == ''){ /*   This only runs if the above two do not find any results even when there should be. Fo instance, searching for die, hard, or vengence will not show up in the search results. but if you search for 'die hard' specifically, it shows up. Thererfore this function will search for the first two words of the title      */
                         ...
                         /*    echo urldecode($link5);  */
                               ...
                               /*  echo $newlink2;   */
                               $link11 =   ($link7 . str_replace(' ','%20',$newlink4) . $link8);               
                                              /*  echo $link11;  */
                                                        ...
                                                        foreach($b3 as $ween3=>$peen3){
                                                            ...
                                                            foreach($peen3 as $yeet3=>$yeet33){    
                                                                ...
                                                                if(($yeet33->attributes()->size < 22548578304)){
                                                                    $final4 = ('<p> <a href="vlc://' . substr($link7, 0, -13) . substr($newarr[2], 1) . '?' . substr($link8, 1) . '" style="color:orange;">' . $newarr[0]  . '</a>   || <p style="color:orange;">     FileSize: ' . round($newarr[1]/1024/1024/1024, 2) . 'GB  ||  Duration:  '. round($duration3/60000) . ' mins   ||  BitRate: ' . round($bitrate3/1000, 2)  . ' Mb/s || Video Codec: ' . $videoCodec3 . ' ||  Video Resolution: ' . $videoResolution3 . '</p>');
